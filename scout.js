...
const searchQuery = `${config.product} ${config.brand} deal price under ${config.priceLimit} // STEP 2: Search the web for free using DuckDuckGo (no API key, no account, no card needed)
...
while ((linkMatch = linkRegex.exec(searchHtml)) !== null) links.push({ url: resolveUrl(linkMatch[while ((snippetMatch = snippetRegex.exec(searchHtml)) !== null) snippets.push(snippetMatch[1].for (let i = 0; i < Math.min(links.length, 8); i++) {
results.push({ title: links[i].title, url: links[i].url, snippet: snippets[i] || "" });
}
// STEP 4: Ask Gemini to pick the best matching deals from these real search results
const prompt = `
You are a shopping deal scout. Below are real web search results for a product search. Based - Product: ${config.product}
...
For each matching deal, extract or estimate: title, price (number, or null if not stated), currency, Only include results that are plausibly real product listings, not articles or unrelated pages.
IMPORTANT ACCURACY RULES:
- Only use a "url" value if it comes directly from the SAME search result entry as the title - If you are not fully confident a url belongs to that exact listing, set "url" to the search - Do not invent, guess, or "clean up" any url.
...
2. For the filename, type exactly: .github/workflows/daily-scout.yml (GitHub will autocreate
the folders).
3. Paste:
